...
/* Etiket renkleri temaya gore degisir: koyu zeminde pastel, acik zeminde
   doygun ve koyu tonlar (aksi halde beyazda okunmuyorlar). */
const STATUS: Record<string, string> = {
  capstone: "border-tag-capstone/40 text-tag-capstone",
  team: "border-tag-team/40 text-tag-team",
  research: "border-tag-research/40 text-tag-research",
  live: "border-tag-live/40 text-tag-live",
};
  ...
  return (
    <div ref={root} className="home relative z-0 min-h-screen overflow-x-clip bg-space text-hero-2">
      ...
      <header className="giris-gizle fixed inset-x-0 top-0 z-30 border-b border-hero/5 bg-space/70 backdrop-blur-md">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-3.5 sm:px-8">
          ...
          <nav className="nav-display hidden gap-3.5 text-[12px] text-hero-2 lg:flex lg:gap-7 lg:text-[13px]">
            ...
            <a href="#research" className="transition hover:text-hero">{t.nav.research}</a>
            <a href="#skills" className="transition hover:text-hero">{t.nav.skills}</a>
      ...
      <section id="top" ref={heroRef} className="giris-gizle relative flex min-h-[100svh] items-center overflow-hidden">
        ...
        <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(to_bottom,transparent_55%,rgb(var(--space-rgb)/0.55)_100%)]" />
        ...
        <div aria-hidden className="hero-fade absolute bottom-10 left-1/2 z-10 -translate-x-1/2">
          <span className="block h-16 w-px animate-pulse bg-gradient-to-b from-hero/55 to-transparent" />
        </div>
      </section>
      ...
      <section id="about" data-fit="screen" className="relative mx-auto flex max-w-6xl flex-col justify-center px-5 pt-32 sm:px-8 md:min-h-screen md:pb-[6vh] md:pt-[10vh] md:[@media(max-height:820px)]:pb-[4vh] md:[@media(max-height:820px)]:pt-[7vh]">
        ...
        <div
          ...
          style={{ background: "linear-gradient(to bottom, rgb(var(--space-rgb) / 0.55), transparent)" }}
        />
        ...
        <div className="mt-12 grid grid-cols-4 gap-px overflow-hidden rounded-2xl border border-hero/10 bg-hero/10 md:mt-[5vh] md:[@media(max-height:820px)]:mt-[3.5vh]">
          {t.stats.map((s) => (
            <div key={s.label} className="bg-surface px-2.5 py-4 sm:px-6 sm:py-7 md:sikisik:py-5 md:[@media(max-height:820px)]:py-3.5" data-reveal>
          ...
          >
            <div ref={trackRef} className="flex flex-col gap-4 md:flex md:h-auto md:w-max md:flex-row md:items-stretch md:gap-6 md:pl-[max(1.25rem,calc((100vw-72rem)/2+2rem))] md:pr-[14vw]">
              {t.journey.stops.map((s, i) => (
                <article key={i} className={`journey-card group relative h-[55vh] shrink-0 md:inset-auto flex w-full flex-col overflow-hidden rounded-3xl border p-7 shadow-2xl shadow-black/50 transition-colors md:aspect-[3/4] md:w-auto md:p-8 md:sikisik:p-6 md:[@media(max-height:820px)]:aspect-auto md:[@media(max-height:820px)]:w-[23rem] ${photos.length ? "md:h-[min(50vh,31rem)] md:[@media(max-height:820px)]:h-[43vh]" : "md:h-[min(58vh,33rem)]"} ${s.featured ? "border-brand/45 bg-gradient-to-b from-surface-2 to-surface hover:border-brand/70" : "border-hero/10 bg-surface hover:border-hero/25"}`} data-reveal>
                  {s.featured && <span aria-hidden className="pointer-events-none absolute -left-24 -top-24 h-64 w-64 rounded-full bg-[radial-gradient(circle,rgb(var(--brand-rgb)/0.28),transparent_65%)]" />}
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-2">
          {t.projects.cards.map((c) => (
            <a
              ...
              onMouseLeave={untilt}
              data-reveal
              className="group relative rounded-2xl border border-hero/10 bg-surface p-7 transition-transform duration-200 [transform:perspective(900px)_rotateX(var(--rx,0deg))_rotateY(var(--ry,0deg))] hover:border-hero/25"
            >
              <div className="absolute inset-0 rounded-2xl bg-[radial-gradient(circle_at_var(--mx,50%)_var(--my,50%),rgb(var(--brand-rgb)/0.12),transparent_55%)] opacity-0 transition group-hover:opacity-100" />
          ...
          <div>
            ...
            <div className="mt-[4.5rem] flex justify-center md:justify-start md:sikisik:mt-12 md:[@media(max-height:820px)]:mt-7">
              <div className="stamp relative inline-flex rotate-[-8deg] flex-col items-center gap-2 rounded-lg border-[3px] border-brand-soft/85 px-9 py-5 sikisik:px-7 sikisik:py-4 max-[400px]:px-5 max-[400px]:py-3.5 text-center">
                ...
                <span className="font-display text-[clamp(1.9rem,4.4vw,2.7rem)] max-[400px]:text-[1.45rem] font-semibold uppercase leading-none tracking-[0.1em] whitespace-nowrap text-brand-soft">{t.research.highlight.title}</span>
                <span className="text-[10.5px] max-[400px]:text-[9px] font-semibold uppercase tracking-[0.3em] whitespace-nowrap text-brand-soft/80 sikisik:tracking-[0.22em] max-[400px]:tracking-[0.16em]">{t.research.highlight.label}</span>
              </div>
            </div>
          </div>
      ...
      <section id="skills" className="mx-auto max-w-6xl px-5 pt-32 sm:px-8">
        ...
        <div className="mt-14 flex items-center gap-4" data-reveal>
          <span className="text-[11px] font-semibold uppercase tracking-[0.22em] text-brand-soft">{t.skills.humanLabel}</span>
          <span className="h-px flex-1 bg-gradient-to-r from-brand/40 to-transparent" />
        </div>
